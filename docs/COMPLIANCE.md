...
| **Execution-partner adapters do not move Meridian-held funds** | `executionPartnerAdapters` is true (sandbox mocks exist). `delegateExecution` stays false. HTTP dispatch forwards a **caller-signed** instruction; Meridian stores `instructionHash` / `signatureHash` only. The mock partner reports `accepted → settling → settled` (or `failed` / `partial`). `fundsMoved` and `meridianKeysUsed` stay false. Live adapters are fail-closed behind `PARTNER_LIVE_ENABLED` (default false) and none are implemented. |
| **Sandbox execution orchestration is not custody or live settlement** | `sandboxExecutionOrchestration` is true (code exists). HTTP is fail-closed behind `EXECUTION_ENABLED` (default false) → `POST /executions` stays 501. When on, sandbox mock partners only. Meridian HMAC-signs the instruction with a vault partner-scoped credential; `transferSigned` stays false (not a funds transfer). `fundsMoved` / `custody` / `meridianKeysUsed` stay false. Production-locked processes cannot enable the flag. Live partners are never called. `executeTransactions` and `delegateExecution` stay false. |
| **Verifiable receipts are not fund-transfer signatures** | `verifiableExecutionReceipts` is true. HTTP is fail-closed behind `EXECUTION_ENABLED`. Meridian Ed25519-signs a canonical receipt (mandate hashes, route rationale, partner confirmation hashes, timestamps). The private key stays in the vault (`meridian-receipt-signer` / `receipt_ed25519`). `transferSigned` on the payload stays false. No raw PII. |
| **Reconciliation flags mismatches; it does not settle** | `settlementReconciliation` is true. HTTP is fail-closed behind `EXECUTION_ENABLED`. `GET /reconciliation/mismatches` matches dispatched instruction ↔ partner confirmation ↔ fee attribution and flags disagreements. `fundsMoved` stays false. |
| **Audit export is tenant-scoped** | `auditTrailExport` is true. `GET /audit/export` is owner/admin session only and selects `organization_id` from the principal. Wrong tenant is never returned. |
...
`mandate.revoked`, `partner.dispatched`, `partner.status_changed`, `partner.failed`. The audit repository exposes no update or delete operation.
