...
| **No AI-agent real payment execution**                         | `agentPayments` and `agentPaymentSimulation` are true: agents create intents, quote, authorize and run the sandbox simulator. `COMPLETED` is simulated. `fundsMoved` stays false. Principals of kind `agent` (`mag_` credentials) still act *for* an organization. Wallet references are external handles; `controlledByPlatform` is always false. `POST /executions` remains 501. |
