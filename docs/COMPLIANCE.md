...
| **Execution-partner adapters do not move Meridian-held funds** | `executionPartnerAdapters` is true (sandbox mocks exist). `delegateExecution` stays false. HTTP dispatch forwards a **caller-signed** instruction; Meridian stores `instructionHash` / `signatureHash` only. The mock partner reports `accepted → settling → settled` (or `failed` / `partial`). `fundsMoved` and `meridianKeysUsed` stay false. Live adapters are fail-closed behind `PARTNER_LIVE_ENABLED` (default false) and none are implemented. `POST /executions` remains 501. |
