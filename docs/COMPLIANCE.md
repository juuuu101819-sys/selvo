...
| **No DeFi execution**                                          | `defiExecution` is false. `defiQuotes` is true: a demo AMM/aggregator returns read-only depth. No adapter is registered as a `RouteProvider`. No swaps, wraps, bridges, keys or wallets. |
