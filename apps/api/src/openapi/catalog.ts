...
export const API_V1_ROUTE_CATALOG: readonly CatalogRoute[] = [
  ...
  r(
    ...
    'authenticated',
    'transaction:create',
    'Generate and return a signed settlement instruction. Meridian hands the artifact to the customer and does not transmit it to any provider. POST /executions stays 501.',
    ['Settlement instructions'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'transaction:create',
    'Read a generated instruction with its verification material. Reports expiry and quote staleness rather than serving a stale artifact silently.',
    ['Settlement instructions'],
  ),
  r(
    'POST',
    ...
    'authenticated',
    'transaction:create',
    "Store the customer's counter-signature for audit. Storing it triggers no dispatch: the customer executes through their own provider relationship.",
    ['Settlement instructions'],
  ),
  r(
    'POST',
    '/settlement/instructions/verify',
    'public',
    'none',
    'Verify a Meridian instruction signature. A convenience only — the artifact carries everything needed to verify offline against the published JWKS.',
    ['Settlement instructions'],
  ),
  r(
    'GET',
    ...
    'public',
    'none',
    'JWKS of Meridian instruction signing keys. Public so a customer provider can verify without a Meridian credential. Retired keys stay listed until the instructions they signed expire.',
    ['Settlement instructions'],
  ),
  r(
    'GET',
    ...
    'public',
    'none',
    'How to verify an instruction, and what the Meridian signature does and does not attest. It is authenticity of a recommendation, never authorization to move funds.',
    ['Settlement instructions'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'transaction:create',
    'Read one recorded execution intent. Still executable=false and submitted=false.',
    ['Execution intents'],
  ),
  ...
  r(
    ...
    'authenticated',
    'onboarding_operator',
    'Engage a provider, corridor, or region kill switch. Required reason. Audited. Does not auto-reset.',
    ['Ops'],
  ),
  r(
    'POST',
  ...
  r(
    ...
    'authenticated',
    'onboarding_operator',
    'List per-corridor, per-partner, and billing live flags. Sign-off metadata required to enable. Default sandbox.',
    ['Ops'],
  ),
  r(
    'POST',
    ...
    'authenticated',
    'onboarding_operator',
    'Record live enablement. Refuses without complete legal/licensing sign-off. Does not move funds.',
    ['Ops'],
  ),
  r(
    'POST',
    '/ops/live-enablement/disable',
    'authenticated',
    'onboarding_operator',
    'Disable a live-enablement row. Sandbox is retained. Audited.',
    ['Ops'],
  ),
  r(
    'GET',
    '/ops/live-enablement/evaluate',
    'authenticated',
    'onboarding_operator',
    'Evaluate live funds-movement and billing admission. Fail-closes on missing sign-off, licence expiry, or region kill switch.',
    ['Ops'],
  ),
  r(
    'POST',
    ...
    'authenticated',
    'onboarding_operator',
    'Attempt platform-fee collection. Refused unless BILLING_LIVE_ENABLED and sign-off; this tree never collects.',
    ['Billing'],
  ),
  r(
    'POST',
    '/ops/billing/subscriptions/run',
    'authenticated',
    'onboarding_operator',
    'Attempt live subscription billing. Fail-closed behind BILLING_LIVE_ENABLED. Never charges.',
    ['Billing'],
  ),
  r(
    'POST',
    '/ops/billing/partner-payouts/run',
    'authenticated',
    'onboarding_operator',
    'Attempt partner payouts. Fail-closed behind BILLING_LIVE_ENABLED. Never creates payable balances.',
    ['Billing'],
  ),
  r(
    'POST',
    ...
    'authenticated',
    'onboarding_operator',
    'Assign an organization a subscription tier. Placeholder prices; assigning a tier charges nobody.',
    ['Billing'],
  ),
  r(
    'POST',
    '/ops/billing/invoices/:id/collect',
    'authenticated',
    'onboarding_operator',
    'Collect one invoice. RECORD_ONLY records the attempt without contacting a processor; LIVE requires the billing sign-off. Idempotent per invoice.',
    ['Billing'],
  ),
  r(
    'GET',
    '/ops/billing/collection',
    'authenticated',
    'onboarding_operator',
    'Current collection mode and the reasons live collection is or is not active.',
    ['Billing'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'organization',
    'Metered API calls for the period, the tier quota, and the overage that would be invoiced. Not cash received.',
    ['Dashboard'],
  ),
  r(
    'GET',
  ...
  r(
    ...
    'public',
    'none',
    '501 when EXECUTION_ENABLED is false. Sandbox orchestration against mock partners when the flag is on. Never live, never custody.',
    ['Execution'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'organization',
    'Organization-scoped orchestration status. Wrong tenant 404. Flag off is 403.',
    ['Execution'],
  ),
  r(
    'GET',
    '/executions/:id/receipt',
    'authenticated',
    'organization',
    'Verifiable Ed25519 receipt for a SETTLED sandbox orchestration. Hashes only; wrong tenant 404.',
    ['Execution'],
  ),
  r(
    'POST',
    ...
    'public',
    'none',
    'Independently verify a presented execution receipt with its public key. Does not fetch tenant data.',
    ['Execution'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'organization',
    'Flag dispatched-instruction vs partner-confirmation vs fee-attribution mismatches. Flag off is 403.',
    ['Execution'],
  ),
  r(
    'GET',
    ...
    'authenticated',
    'owner_admin',
    'Tenant-scoped audit trail export for owner/admin. Wrong tenant never selected.',
    ['Audit'],
  ),
];
